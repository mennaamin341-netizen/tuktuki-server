...
"""
import os, re, time, uuid, asyncio, urllib.parse
...
API_KEY = os.getenv("API_KEY", "")                 # set as a Space secret
...
async def tts_batch(body: dict, request: Request, x_api_key: str = Header(default="")):
    ...
    style = (body.get("style_prompt") or "").strip()
    ...
    async def do_image(client, i, s):
        prompt = ", ".join(x for x in [style, s.get("image_prompt", "")] if x)
